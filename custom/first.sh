...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/2025op/common build/common
...
git clone -b main --depth 1 https://github.com/2025op/build-actions shangyou
...
if [[ "${GIT_REPOSITORY}" == "2025op/build-actions" ]]; then
  rm -rf backups
  BANBEN_SHUOMING="Update $(date +%Y.%m%d.%H%M.%S)"
fi
if [[ "${GIT_REPOSITORY}" == "2025op/autobuild" ]]; then
  rm -rf backups
fi
git add .
...
curl -fsSL https://raw.githubusercontent.com/2025op/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/2025op/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="2025op/build-actions"
  sudo rm -rf build common.sh
