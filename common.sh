#!/bin/bash
# https://github.com/2025op/build-actions
...
cd ${GITHUB_WORKSPACE}
bash <(curl -fsSL https://raw.githubusercontent.com/2025op/common/refs/heads/main/custom/first.sh)
}
...
function Diy_update() {
bash <(curl -fsSL https://raw.githubusercontent.com/2025op/common/main/custom/ubuntu.sh)
if [[ $? -ne 0 ]];then
...
gitsvn https://github.com/2025op/common/tree/main/auto-scripts ${HOME_PATH}/package/auto-scripts
...
echo "src-git danshui https://github.com/2025op/openwrt-package.git;$SOURCE" >> ${HOME_PATH}/feeds.conf.default
echo "src-git dstheme https://github.com/2025op/openwrt-package.git;$THEME_BRANCH" >> ${HOME_PATH}/feeds.conf.default
...
if [[ -z "$(find "$A_PATH" -type d -name "default-settings" -print)" ]] && [[ -d "$C_PATH" ]]; then
  gitsvn https://github.com/2025op/common/tree/main/Share/default-settings ${HOME_PATH}/package/default-settings
  ...
  gitsvn https://github.com/2025op/common/tree/main/Share/default-setting ${HOME_PATH}/package/default-settings
  if grep -q "libustream-wolfssl" "${HOME_PATH}/include/target.mk"; then
    sed -i 's?libustream-wolfssl?libustream-openssl?g' "${HOME_PATH}/include/target.mk"
  fi
  if ! grep -q "dnsmasq-full" "${HOME_PATH}/include/target.mk"; then
...
if [[ "${REPO_BRANCH}" == *"18.06"* ]] || [[ "${REPO_BRANCH}" == *"19.07"* ]] || [[ "${REPO_BRANCH}" == *"21.02"* ]] || [[ "${REPO_BRANCH}" == *"22.03"* ]]; then
   gitsvn https://github.com/2025op/common/blob/main/Share/shadowsocks-rust/Makefile ${HOME_PATH}/feeds/danshui/luci-app-ssr-plus/shadowsocks-rust/Makefile
fi
...
if [[ ! -d "${HOME_PATH}/feeds/packages/devel/packr" ]]; then
  gitsvn https://github.com/2025op/common/tree/main/Share/packr ${HOME_PATH}/feeds/packages/devel/packr
fi
...
if [[ "${REPO_BRANCH}" =~ (main|master|openwrt-24.10) ]]; then
  gitsvn https://github.com/2025op/common/blob/main/Share/luci-app-nginx-pingos/Makefile ${HOME_PATH}/feeds/danshui/luci-app-nginx-pingos/Makefile
fi
}
...
' >> ${HOME_PATH}/.config
gitsvn https://github.com/2025op/common/blob/main/Share/block/10-mount ${HOME_PATH}/files/etc/hotplug.d/block/10-mount
fi
...
if [[ `grep -c "CONFIG_PACKAGE_luci-theme-argon=y" ${HOME_PATH}/.config` -eq '1' ]]; then
  ...
  curl -fsSL https://raw.githubusercontent.com/2025op/common/main/Share/argon/jpg/${pmg}.jpg -o ${HOME_PATH}/files/www/luci-static/argon/background/argon.jpg
  if [[ $? -ne 0 ]]; then
    echo "拉取文件错误,请检测网络"
    exit 1
  fi
...
if [[ ! "${weizhicpu}" == "1" ]] && [[ "${AdGuardHome_Core}" == "1" ]]; then
  ...
  wget -q https://github.com/2025op/common/releases/download/API/AdGuardHome.api -O AdGuardHome.api
  if [[ $? -ne 0 ]];then
    curl -fsSL https://github.com/2025op/common/releases/download/API/AdGuardHome.api -o AdGuardHome.api
  fi
...
if [[ -z "${amlogic_kernel}" ]]; then
  curl -fsSL https://github.com/2025op/common/releases/download/API/${kernel_usage}.api -o ${HOME_PATH}/${kernel_usage}.api
